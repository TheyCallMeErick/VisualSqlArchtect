...
public class ExplainPlanExportViewModelTests
{
    ...
    {
        var canvas = new CanvasViewModel();
        canvas.ActiveConnectionConfig = new AkkornStudio.Core.ConnectionConfig(
            Provider: AkkornStudio.Core.DatabaseProvider.SQLite,
            Host: string.Empty,
            Port: 0,
            Database: "db.sqlite",
        ...
        SetPrivateField(sut, "_provider", AkkornStudio.Core.DatabaseProvider.SQLite);
        SetPrivateField(sut, "_rawOutput", "{\"Plan\":{}}");
