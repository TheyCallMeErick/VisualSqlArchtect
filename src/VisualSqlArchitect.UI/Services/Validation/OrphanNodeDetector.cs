...
namespace VisualSqlArchitect.UI.Services.Validation;
...
public static class OrphanNodeDetector
{
    ...
    public static IReadOnlySet<string> DetectOrphanIds(CanvasViewModel canvas)
    {
        ...
        // Orphans = every node NOT reached from a sink
        return canvas.Nodes.Where(n => !reachable.Contains(n.Id)).Select(n => n.Id).ToHashSet();
    }
}
