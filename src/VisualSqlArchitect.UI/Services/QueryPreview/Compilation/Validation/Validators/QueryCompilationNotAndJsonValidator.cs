...
internal sealed class QueryCompilationNotAndJsonValidator(CanvasViewModel canvas)
{
    ...
    private HashSet<string> CollectActiveSelectNodeIds(NodeViewModel resultOutputNode)
    {
        ...
        foreach (ConnectionViewModel conn in _canvas.Connections.Where(c =>
            c.ToPin?.Owner == resultOutputNode
            && c.ToPin.Name.Equals("column", StringComparison.OrdinalIgnoreCase)))
        {
            selectRoots.Add(conn.FromPin.Owner.Id);
        }
        ...
        foreach (ConnectionViewModel columnSetConn in _canvas.Connections.Where(c =>
            c.ToPin?.Owner == resultOutputNode
            && c.ToPin.Name.Equals("columns", StringComparison.OrdinalIgnoreCase)
            && c.FromPin.Owner.Type is NodeType.ColumnList or NodeType.ColumnSetBuilder))
        {
            NodeViewModel columnSetNode = columnSetConn.FromPin.Owner;
            foreach (ConnectionViewModel selectConn in _canvas.Connections.Where(c =>
                c.ToPin?.Owner == columnSetNode
                && IsProjectionInputPinName(c.ToPin.Name)))
            {
                selectRoots.Add(selectConn.FromPin.Owner.Id);
            }
        }
