...
public sealed class CteValidator : IGraphValidator
{
    ...
    private void ValidateCteSources(List<string> errors)
    {
        ...
        foreach (NodeViewModel definition in _canvas.Nodes.Where(n => n.Type == NodeType.CteDefinition))
        {
            string? definitionName = ResolveDefinitionName(definition);
            if (string.IsNullOrWhiteSpace(definitionName) || defined.Contains(definitionName))
                continue;

            if (CanResolveDefinitionToFromSource(definition))
                defined.Add(definitionName);
        }
    ...
    private bool CanResolveDefinitionToFromSource(NodeViewModel definition)
    {
        ...
        var upstream = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { cteOutput.Id };
        var queue = new Queue<string>();
        queue.Enqueue(cteOutput.Id);
        while (queue.Count > 0)
        {
            string current = queue.Dequeue();
            foreach (ConnectionViewModel conn in _canvas.Connections.Where(c => c.ToPin?.Owner.Id == current))
            {
                string ownerId = conn.FromPin.Owner.Id;
                if (upstream.Add(ownerId))
                    queue.Enqueue(ownerId);
            }
        }

        return _canvas.Nodes.Any(n =>
