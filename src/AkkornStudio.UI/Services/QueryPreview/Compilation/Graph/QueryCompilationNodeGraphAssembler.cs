    ...
    QueryCompilationCteResolver cteResolver)
{
    ...
    )
    {
        ...
        var nodes = scopedNodes.Select(n => new AkkornStudio.Nodes.NodeInstance(
                Id: n.Id,
                Type: n.Type,
        ...
        var selectBindings = new List<AkkornStudio.Nodes.SelectBinding>();

        selectBindings.AddRange(
            CollectProjectedPins(_canvas, resultOutputNode)
                .Select(pin => new AkkornStudio.Nodes.SelectBinding(pin.Owner.Id, pin.Name, pin.Owner.Alias)));

        var whereBindings = _canvas
            .Connections.Where(c => c.ToPin?.Owner == resultOutputNode && c.ToPin?.Name == "where")
            .Select(c => new AkkornStudio.Nodes.WhereBinding(
                c.FromPin.Owner.Id,
                c.FromPin.Name
            ))
            .ToList();
        ...
        var havingBindings = _canvas
            ...
            .Select(c => new AkkornStudio.Nodes.HavingBinding(
                c.FromPin.Owner.Id,
                c.FromPin.Name
            ))
            .ToList();
        ...
        var qualifyBindings = _canvas
            ...
            .Select(c => new AkkornStudio.Nodes.QualifyBinding(
                c.FromPin.Owner.Id,
                c.FromPin.Name
            ))
            .ToList();
        ...
        var orderBys = new List<AkkornStudio.Nodes.OrderBinding>();
        orderBys.AddRange(
            _canvas.Connections
                .Where(c => c.ToPin?.Owner == resultOutputNode && c.ToPin.Name.Equals("order_by", StringComparison.OrdinalIgnoreCase))
                .Select(c => new AkkornStudio.Nodes.OrderBinding(c.FromPin.Owner.Id, c.FromPin.Name, Descending: false)));
        orderBys.AddRange(
            _canvas.Connections
                .Where(c => c.ToPin?.Owner == resultOutputNode && c.ToPin.Name.Equals("order_by_desc", StringComparison.OrdinalIgnoreCase))
                .Select(c => new AkkornStudio.Nodes.OrderBinding(c.FromPin.Owner.Id, c.FromPin.Name, Descending: true)));

        var groupBys = new List<AkkornStudio.Nodes.GroupByBinding>();
        groupBys.AddRange(
            _canvas.Connections
                .Where(c => c.ToPin?.Owner == resultOutputNode && c.ToPin.Name.Equals("group_by", StringComparison.OrdinalIgnoreCase))
                .Select(c => new AkkornStudio.Nodes.GroupByBinding(c.FromPin.Owner.Id, c.FromPin.Name)));
        ...
        List<AkkornStudio.Nodes.CteBinding> ctes = includeCtes
    ...
    private List<AkkornStudio.Nodes.CteBinding> BuildCtes(
        IReadOnlyList<NodeViewModel> cteDefinitions,
        IReadOnlyDictionary<string, string> cteDefinitionNamesById
    )
    {
        var ctes = new List<AkkornStudio.Nodes.CteBinding>();
        ...
        foreach (NodeViewModel definition in cteDefinitions)
        {
            ...
            ctes.Add(new AkkornStudio.Nodes.CteBinding(name, fromTable, cteGraph, recursive));
        }
