...
internal sealed class QueryCompilationGenerationErrorMapper
{
    ...
    {
        if (TryResolvePlanningException(ex, out AkkornStudio.Nodes.LogicalPlan.PlanningException? planningException))
        {
            AkkornStudio.Nodes.LogicalPlan.PlanningException resolved = planningException!;
            yield return resolved.Message;

            switch (resolved.Kind)
            {
                case AkkornStudio.Nodes.LogicalPlan.PlannerErrorKind.JoinWithoutCondition:
                    yield return "Join node requires an explicit condition. Connect a condition pin or configure an explicit ON expression.";
                    yield break;
                case AkkornStudio.Nodes.LogicalPlan.PlannerErrorKind.OutputSourceAmbiguous:
                    yield return "Use exactly one top-level Result Output and ensure it references a single resolvable source path.";
                    yield break;
                case AkkornStudio.Nodes.LogicalPlan.PlannerErrorKind.DatasetNotReachableFromOutput:
                    yield return "Output is not connected to a reachable dataset source. Connect table/cte/subquery sources to output columns.";
                    yield break;
                case AkkornStudio.Nodes.LogicalPlan.PlannerErrorKind.DuplicateAlias:
                    yield return "Duplicate dataset alias detected. Set unique aliases for each source node.";
                    yield break;
                case AkkornStudio.Nodes.LogicalPlan.PlannerErrorKind.CteNotReferencedInPlan:
                    yield return "CTE definida sem uso no plano final. Conecte um CTE Source ao fluxo que chega no Result Output.";
                    yield break;
                case AkkornStudio.Nodes.LogicalPlan.PlannerErrorKind.CyclicDependency:
    ...
    private static bool TryResolvePlanningException(
        Exception exception,
        out AkkornStudio.Nodes.LogicalPlan.PlanningException? planningException)
    {
        if (exception is AkkornStudio.Nodes.LogicalPlan.PlanningException direct)
        {
            planningException = direct;
            return true;
        }

        if (exception.InnerException is AkkornStudio.Nodes.LogicalPlan.PlanningException inner)
        {
