...
public sealed class JoinResolver
{
    ...
    private bool TryBuildExplicitJoinFromParameters(NodeViewModel joinNode, out JoinDefinition join, out string? warning)
    {
        ...
        string rightSource = joinNode.Parameters.TryGetValue("right_source", out string? target)
            ...
            : string.Empty;
        string onRaw = joinNode.Parameters.TryGetValue("on_raw", out string? onRawValue)
            ? (onRawValue ?? string.Empty).Trim()
            : string.Empty;
        ...
        out string? warning)
    {
        ...
        string? onRaw = TryCompileConditionNodeToSql(conditionNode);
        if (string.IsNullOrWhiteSpace(onRaw))
            onRaw = TryCompileNodeExpressionToSql(conditionNode, "result");
        if (string.IsNullOrWhiteSpace(onRaw))
        {
    ...
    private string? TryCompileConditionNodeToSql(NodeViewModel node)
    {
        if (node.Type is not (NodeType.And or NodeType.Or or NodeType.Not))
            return null;

        if (node.Type == NodeType.Not)
        {
            ConnectionViewModel? child = _canvas.Connections.FirstOrDefault(c =>
                c.ToPin?.Owner == node
                && c.ToPin.Name.Equals("condition", StringComparison.OrdinalIgnoreCase));
            if (child is null)
                return null;

            string? childSql = TryCompileNodeExpressionToSql(child.FromPin.Owner, child.FromPin.Name);
        ...
        IReadOnlyList<ConnectionViewModel> conditionWires = _canvas.Connections
            .Where(c =>
                c.ToPin?.Owner == node
                && c.ToPin.Name.Equals("conditions", StringComparison.OrdinalIgnoreCase))
            .ToList();
        if (conditionWires.Count == 0)
            return null;

        var parts = new List<string>(conditionWires.Count);
        foreach (ConnectionViewModel wire in conditionWires)
        {
            string? partSql = TryCompileNodeExpressionToSql(wire.FromPin.Owner, wire.FromPin.Name);
        ...
        string op = node.Type == NodeType.And ? " AND " : " OR ";
    ...
    private string? TryCompileNodeExpressionToSql(NodeViewModel node, string pinName)
    {
        try
        {
            ...
            var graphNodes = allNodes.Select(n => new AkkornStudio.Nodes.NodeInstance(
                    Id: n.Id,
                    Type: n.Type,
            ...
            var graphConnections = _canvas.Connections
                ...
                .Select(c => new AkkornStudio.Nodes.Connection(
                    c.FromPin.Owner.Id,
                    c.FromPin.Name,
                    c.ToPin!.Owner.Id,
                    c.ToPin.Name
            ...
            var graph = new NodeGraph
            {
                ...
                SelectOutputs = [new AkkornStudio.Nodes.SelectBinding(node.Id, pinName)],
            };
    ...
    private bool TryResolveJoinOperand(NodeViewModel owner, string pinName, out string expr)
    {
        ...
        if (owner.Type is NodeType.Subquery or NodeType.SubqueryReference)
        {
