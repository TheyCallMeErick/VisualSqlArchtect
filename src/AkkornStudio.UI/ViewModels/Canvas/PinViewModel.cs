...
public sealed class PinViewModel(PinDescriptor d, NodeViewModel owner) : ViewModelBase
{
    ...
    public AkkornStudio.Nodes.Pins.PinModel? Model { get; private set; }
    ...
    internal void ResetAbsolutePosition()
    {
        if (!_hasAbsolutePosition && _absolutePosition == default)
            return;

        _absolutePosition = default;
        _hasAbsolutePosition = false;
    ...
    public PinViewModel(AkkornStudio.Nodes.Pins.PinModel model, NodeViewModel owner)
